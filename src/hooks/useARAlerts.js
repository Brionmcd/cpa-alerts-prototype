...
import { arAlertsApi } from '../services/api'

/**
 * Custom hook for managing AR alerts
 *
 * Provides:
 * - alerts: Array of AR alerts
...
export function useARAlerts() {
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await arAlertsApi.getAll()
      setAlerts(data)
    } catch (err) {
      setError(err.message || 'Failed to load AR alerts')
      console.error('Error fetching AR alerts:', err)
    } finally {
      setIsLoading(false)
    }
  }, [])
  ...
  const markHandled = useCallback(async (id, note = null) => {
    try {
      await arAlertsApi.markHandled(id, note)
  ...
  const snooze = useCallback(async (id, days) => {
    try {
      await arAlertsApi.snooze(id, days)
  ...
  const dismiss = useCallback(async (id, reason) => {
    try {
      await arAlertsApi.dismiss(id, reason)
      // Optimistically remove from local state
      setAlerts(prev => prev.filter(a => a.id !== id))
      return { success: true }
    } catch (err) {
...
/**
 * Hook for getting a single AR alert by ID
 */
export function useARAlertDetail(id) {
  ...
  useEffect(() => {
    ...
    const fetchAlert = async () => {
      ...
      setError(null)
      try {
        const data = await arAlertsApi.getById(id)
        setAlert(data)
      } catch (err) {
        setError(err.message || 'Failed to load alert details')
        console.error('Error fetching AR alert detail:', err)
      } finally {
        setIsLoading(false)
      }
    }
